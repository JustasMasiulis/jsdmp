...
const enum MinidumpMiscInfoFlags1 {
...
const enum ThreadPriorityClass {
...
export function priorityToString(priorityClass: number, priority: number): string {
...
export class MiniDump {
	...
	constructor(data: ArrayBuffer) {
		...
		for (let i = 0; i < numStreams; i++) {
			...
			const streamData = this._data.buffer.slice(streamRva, streamRva + streamSize);
			...
			if (streamType === MiniDumpStreamType.ThreadInfoListStream) {
				this.threadInfoList = this.parseThreadInfoListStream(streamRva, streamSize);
			}
	...
	private parseThreadListStream(streamRva: number, streamSize: number): MinidumpThread[] {
		if (streamSize < 4) {
	...
	private parseExceptionStream(
		...
		for (let i = 0; i < numberParameters; i++) {
			exceptionInformation.push(view.getBigUint64(informationOffset + i * 8, true));
		}
	...
	private parseMiscInfoStream(streamRva: number, streamSize: number): MinidumpMiscInfo {
		if (streamSize < 8) {
		...
		const hasProcessTimes = (flags1 & MinidumpMiscInfoFlags1.ProcessTimes) !== 0;
		...
		return {
			...
			processorMaxMhz:
				hasProcessorPower && readableSize >= 28 ? view.getUint32(24, true) : null,
			processorCurrentMhz:
				hasProcessorPower && readableSize >= 32 ? view.getUint32(28, true) : null,
			processorMhzLimit:
				hasProcessorPower && readableSize >= 36 ? view.getUint32(32, true) : null,
			processorMaxIdleState:
				hasProcessorPower && readableSize >= 40 ? view.getUint32(36, true) : null,
			processorCurrentIdleState:
				hasProcessorPower && readableSize >= 44 ? view.getUint32(40, true) : null,
		};
	}

	private parseSystemInfoStream(streamRva: number, streamSize: number): MinidumpSystemInfo {
		if (streamSize < 56) {
			throw new Error(`SystemInfoStream is too small: ${streamSize} bytes`);
		}

		const view = new DataView(this._data.buffer, this._data.byteOffset + streamRva, streamSize);
		...
		return {
			...
			processorArchitectureName: this.resolveProcessorArchitecture(processorArchitecture),
			processorLevel,
			processorRevision,
			numberOfProcessors,
			productType,
	...
	private readFixedAscii(view: DataView, offset: number, length: number): string {
	...
	private readMinidumpString(rva: number): string {
		...
		const bytes = new Uint8Array(this._data.buffer, this._data.byteOffset + rva + 4, length);
