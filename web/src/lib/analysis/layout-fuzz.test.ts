...
function findEdgeNodeOverlap(
	input: TriskelInput,
	result: ReturnType<typeof triskelLayout>,
			...
			for (let n = 0; n < input.nodeCount; n++) {
				...
				if (
					segmentIntersectsRect(
						...
						result.xs[n] + INSET,
						result.ys[n] + INSET,
						input.nodeWidths[n] - 2 * INSET,
						input.nodeHeights[n] - 2 * INSET,
					)
				) {
...
function assertClean(
	...
	if (overlap) {
		...
		const nw = input.nodeWidths[overlap.node];
		const nh = input.nodeHeights[overlap.node];
		throw new Error(
