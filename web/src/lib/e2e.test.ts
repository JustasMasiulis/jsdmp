...
describe("AMD64 control flow", () => {
	it("ret", () => {
		const d = decode(ProcessorArch.ARCH_AMD64, [0xc3]);
		expect(d.controlFlow.kind).toBe("return");
	});

	it("call with direct target", () => {
		const d = decode(
			ProcessorArch.ARCH_AMD64,
...
describe("AMD64 mnemonic strings", () => {
	...
	for (const [bytes, expected] of cases) {
		it(expected, () => {
			expect(decode(ProcessorArch.ARCH_AMD64, bytes).mnemonic).toBe(expected);
		});
	}
});
...
describe("AMD64 prefix in mnemonic", () => {
	it("lock prefix", () => {
		const d = decode(ProcessorArch.ARCH_AMD64, [0xf0, 0x0f, 0xb1, 0x11]);
		expect(d.mnemonic).toContain("lock");
	});

	it("rep prefix", () => {
		const d = decode(ProcessorArch.ARCH_AMD64, [0xf3, 0xa4]);
...
describe("AMD64 formatted instruction text", () => {
	it("mov has register operands", () => {
		const d = decode(ProcessorArch.ARCH_AMD64, [0x48, 0x89, 0xd8]);
...
describe("ARM64 control flow", () => {
	it("ret", () => {
		const d = decode(ProcessorArch.ARCH_ARM64, [0xc0, 0x03, 0x5f, 0xd6]);
		expect(d.controlFlow.kind).toBe("return");
	});

	it("bl with direct target", () => {
		const d = decode(
			ProcessorArch.ARCH_ARM64,
			[0x05, 0x00, 0x00, 0x94],
			0x1000n,
		);
		expect(d.controlFlow.kind).toBe("call");
...
describe("ARM64 mnemonic strings", () => {
	...
	for (const [bytes, expected] of cases) {
		it(expected, () => {
			expect(decode(ProcessorArch.ARCH_ARM64, bytes).mnemonic).toBe(expected);
		});
	}
});
...
describe("cross-arch decode_length", () => {
	it("AMD64 ret = 1", () => {
		expect(
			decodeInstructionLength(new Uint8Array([0xc3]), ProcessorArch.ARCH_AMD64),
		).toBe(1);
	});
