...
const summarizeMemoryList = (ranges: DebugMemoryRange[]): MemoryListSummary | null => {
	...
	return { rangeCount: ranges.length, totalBytes, startAddress, endAddressExclusive };
};

const buildCodeViewColumns = (cvi: DebugCodeViewInfo | null): [string, string, string, string] => {
	if (!cvi) return [emptyCell, emptyCell, emptyCell, emptyCell];
	switch (cvi.format) {
		case "RSDS": return ["RSDS", cvi.pdbFileName || emptyCell, cvi.guid, String(cvi.age)];
		case "NB10": return ["NB10", cvi.pdbFileName || emptyCell,
			`${fmtHex(cvi.timestamp, 8)} @ ${fmtHex(cvi.offset, 8)}`, String(cvi.age)];
		case "unknown": return [cvi.signature, emptyCell, fmtHex(cvi.rawSignature, 8), emptyCell];
		case "invalid": return ["invalid", cvi.error, emptyCell, emptyCell];
	}
};

const buildModuleRows = (modules: DebugModule[]): string[][] =>
	modules.map((m) => {
		const [cvFormat, cvPdb, cvIdentifier, cvAge] = buildCodeViewColumns(m.codeViewInfo);
		return [
			fmtHex16(m.address), fmtHex8(m.size), fmtHex8(m.checksum),
			fmtHex8(m.timeDateStamp), m.path || emptyCell,
			fmtHex8(m.codeViewRecord.size), cvFormat, cvPdb, cvIdentifier, cvAge,
			fmtHex8(m.miscRecord.size), fmtHex8(m.miscRecord.rva),
		];
	});
...
const buildSummarySection = (dumpInfo: ParsedDumpInfo, container: HTMLElement): void => {
	...
	if (di.miscInfo) {
		...
		if (di.miscInfo.processCreateTime !== null)
			container.append(mkRow("Process Create Time", formatTimestamp(di.miscInfo.processCreateTime)));
		if (di.miscInfo.processUserTime !== null)
			container.append(mkRow("Process User Time", formatSeconds(di.miscInfo.processUserTime)));
		if (di.miscInfo.processKernelTime !== null)
			container.append(mkRow("Process Kernel Time", formatSeconds(di.miscInfo.processKernelTime)));
		if (di.miscInfo.processorMaxMhz !== null)
			container.append(mkRow("CPU Max MHz", String(di.miscInfo.processorMaxMhz)));
		if (di.miscInfo.processorCurrentMhz !== null)
			container.append(mkRow("CPU Current MHz", String(di.miscInfo.processorCurrentMhz)));
		if (di.miscInfo.processorMhzLimit !== null)
			container.append(mkRow("CPU MHz Limit", String(di.miscInfo.processorMhzLimit)));
		if (di.miscInfo.processorMaxIdleState !== null)
			container.append(mkRow("CPU Max Idle State", String(di.miscInfo.processorMaxIdleState)));
		if (di.miscInfo.processorCurrentIdleState !== null)
			container.append(mkRow("CPU Current Idle State", String(di.miscInfo.processorCurrentIdleState)));
	}
};

const buildExceptionSection = (dumpInfo: ParsedDumpInfo, container: HTMLElement): void => {
	const ei = dumpInfo.debugInterface.exceptionInfo;
	if (!ei) return;
	container.append(mkRow("Exception Thread ID", String(ei.threadId)));
	container.append(mkRow("Exception Code", fmtHex(ei.exceptionRecord.exceptionCode, 8)));
	container.append(mkRow("Exception Flags", fmtHex(ei.exceptionRecord.exceptionFlags, 8)));
	container.append(mkRow("Exception Address", fmtHex(ei.exceptionRecord.exceptionAddress, 16)));
	container.append(mkRow("Exception Record", fmtHex(ei.exceptionRecord.exceptionRecord, 16)));
	container.append(mkRow("Exception Parameters", String(ei.exceptionRecord.numberParameters)));
	container.append(mkRow(
		"Exception Context",
		`size=${ei.contextLocation.size}, rva=${fmtHex(ei.contextLocation.rva, 8)}`,
	));
	container.append(mkTable(
		["Index", "Value"],
		buildExceptionParameterRows(ei),
	));
};

const buildModulesSection = (dumpInfo: ParsedDumpInfo, container: HTMLElement): void => {
	const dm = dumpInfo.debugInterface.dm;
	if (dm.modules.length > 0) {
		container.append(mkTable(
			["Base", "Size", "Checksum", "TimeDateStamp", "Name",
				"CV Size", "CV Format", "CV PDB", "CV Identifier", "CV Age",
				"Misc Size", "Misc RVA"],
...
const buildMemorySection = (dumpInfo: ParsedDumpInfo, container: HTMLElement): void => {
	const summary = summarizeMemoryList(dumpInfo.debugInterface.dm.memoryRanges ?? []);
	container.append(mkRow("Memory Ranges", summary ? String(summary.rangeCount) : "0"));
	container.append(mkRow("Memory Bytes", summary ? formatBytes(summary.totalBytes) : "0 B"));
	if (summary) {
		container.append(mkRow(
			"Memory Address Span",
...
export class VanillaDumpSummary {
	...
	private renderThreadsSection(): void {
		...
		const selectedRowIndex = this.threads.findIndex((t) => t.id === this.selectedThreadId);

		const wrap = mkTable(
			[
				"Thread ID", "Suspended", "Priority", "TEB",
				"Stack Start", "Stack Size", "Stack RVA",
				"Context Size", "Context RVA", "Dump Flags", "Dump Error",
				"Exit Status", "Create Time", "Exit Time", "Kernel Time",
				"User Time", "Start Address", "Affinity",
			],
			rows,
			undefined,
			onRowClick,
		...
		this.threadTrs = Array.from(wrap.querySelectorAll<HTMLTableRowElement>("tbody tr"));
